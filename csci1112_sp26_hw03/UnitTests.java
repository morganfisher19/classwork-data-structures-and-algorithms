...
public class UnitTests {
    ...
    public static void main (String[] argv) {
        ...
        // validate that searches do not generate false positives
        int result1 = ClassSchedule.searchIterative(viewSelection, 2);
        int result2 = ClassSchedule.searchBinary(viewSelection, 2);

        if( result1 != -1 ) {
            System.out.println(" searchIterative(...) test 1 failed.");
            return;
        }
        if( result1 != result2 ) {
            System.out.println(" searchBinary(...) test 1 failed.");
            return;
        }

        // validate that searches correctly find a specific record
        result1 = ClassSchedule.searchIterative(viewSelection, 100);
        result2 = ClassSchedule.searchBinary(viewSelection, 100);

        if( result1 != 68 ) {
            System.out.println(" searchIterative(...) test 2 failed.");
            return;
        }
        if( result1 != result2 ) {
            System.out.println(" searchBinary(...) test 2 failed.");
            return;
        }
